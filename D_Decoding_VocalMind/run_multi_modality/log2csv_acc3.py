...
pattern_acc = re.compile(r"acc3: ([\d.]+).*acc1: ([\d.]+)")
...
parser.add_argument('--subjects', nargs='+', type=str, default=['S01'], help='subjects')
...
# root
log_root_dir = '../log_save_multi_modality_vm/' + today
...
for seed in seeds:
    ...
    for line in log_lines:
        ...
        if acc_match and current_subject in subjects:
            acc3 = float(acc_match.group(1))
            acc1 = float(acc_match.group(2))
            all_data[current_subject][seed].append((acc3, acc1))
...
with open(output_csv_file, 'w', newline='') as csvfile:
    ...
    for condition_index in range(max_length):
        ...
        sub_header = [''] + [metric for subject in subjects for metric in ('acc3', 'acc1')] + [condition_value]
        ...
        std_row = ['std']
        for subject in subjects:
            acc3_values = [all_data[subject][seed][condition_index][0] for seed in seeds]
            acc1_values = [all_data[subject][seed][condition_index][1] for seed in seeds]
            avg_row.extend([np.mean(acc3_values), np.mean(acc1_values)])
            std_row.extend([np.std(acc3_values, ddof=1), np.std(acc1_values, ddof=1)])
        avg_row.append('')
        std_row.append('')
