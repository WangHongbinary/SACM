...
LastEditTime: 2026-01-07 10:54:29
Description: Baseline model training code (single_modality)
'''
...
from baseline_models.BrainModule import BrainModule
...
def backbone(args):
    ...
    if args.backbone == 'EEGNet':
        ...
        backbone_model = ShallowConvNet(args,
                                        ...
                                        batch_norm=True,
                                        batch_norm_alpha=0.1)
    elif args.backbone == 'BrainModule':
        backbone_model = BrainModule(data_dim=args.chn, 
                                     time_sample_num=args.time_sample_num, 
                                     out_channels=args.BrainModule_out_channels,
                                     n_classes=args.class_num)
...
def train_baseline(args):
    ...
    # make set
    train_set = MakeSet(train_seeg, label=train_label)
    valid_set = MakeSet(valid_seeg, label=valid_label)
    test_set  = MakeSet(test_seeg, label=test_label)
    ...
    for epoch in range(args.max_epoch):
        ...
        if valid_loss < model_loss:
            ...
            torch.save(model, os.path.join(model_path, f'{args.backbone}.pkl'))
            
        if epoch - last_best_epoch > args.patience:
            break

    '''################################### test ###################################'''
    net_path = os.path.join(model_path, f'{args.backbone}.pkl')
    ...
    for batch_id, data in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            if args.forward_feature:
                forward_feature = model.forward_features(inputs)
                test_features.append(forward_feature.cpu().numpy())
    ...
    if args.forward_feature:
        ...
        np.save(os.path.join(forward_feature_path, f'{args.backbone}_forward_features.npy'), test_features)
        np.save(os.path.join(forward_feature_path, f'{args.backbone}_forward_labels.npy'), test_labels)

    log_print('subject:{} seed:{} test: acc5: {:.2f}, acc1: {:.2f}'.format(args.subject_id, args.seed, test_acck, test_acc1))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--EEGNet_configs', type=int, nargs='+', default=[4, 4, 2, 8], help='EEGNet kernLength F1 D F2')
    ...
    parser.add_argument('--BrainModule_out_channels', type=int, default=256, help='Output channels for BrainModule model')
    ...
    parser.add_argument('--DBConformer_configs', type=int, nargs='+', default=[64, 80, 1, 1], help='emb_size patch_size tem_depth and chn_depth for Conformer')
    ...
    parser.add_argument('--model_path', type=str, default=f'{os.path.dirname(__file__)}/model_save_single_modality/', help='Path to save Model')
    parser.add_argument('--log_path', type=str, default=f'{os.path.dirname(__file__)}/log_save_single_modality/', help='Path to save log')
    parser.add_argument('--matrix_label_path', type=str, default=f'{os.path.dirname(__file__)}/matrix_label_save_single_modality/', help='Classification confusion matrix label save path')
    parser.add_argument('--forward_feature_path', type=str, default=f'{os.path.dirname(__file__)}/forward_feature_save_single_modality/', help='Test forward features save path')
